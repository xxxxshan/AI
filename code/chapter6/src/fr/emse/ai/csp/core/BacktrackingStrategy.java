...
public class BacktrackingStrategy extends SolutionStrategy {
    ...
    private Assignment recursiveBackTrackingSearch(CSP csp,
        ...
        if (assignment.isComplete(csp.getVariables())) {
            ...
            Variable var = degreeH(assignment, csp);
    ...
    protected Variable selectMrv(Assignment assignment, CSP csp){//I don't know how to implementI such an algorithm, cause to judge the mrv will involve calculations later?

    return null;}

    protected Variable degreeH(Assignment assignment, CSP csp) {
        int [] ii = new int[15];
        List<Variable> t=csp.getVariables();
        List<Variable> tt = new ArrayList<Variable>();

            for (Constraint var2 : csp.getConstraints()) {
                List<Variable> scope = var2.getScope();
                ii[t.indexOf(scope.get(0))]=ii[t.indexOf(scope.get(0))]+1;
                ii[t.indexOf(scope.get(1))]=ii[t.indexOf(scope.get(1))]+1;
            ...
            for (int k=0;k<t.size();k++){
                tt.add(ii[k]*csp.getConstraints().size()+k,t.get(k));
            }
            tt.removeAll(Collections.singleton(null));
            for (int i =t.size()-1;i>=0;i--){
                Variable var = tt.get(i);
                if (!(assignment.hasAssignmentFor(var))){
                    return var;
                }
            }
